...
SVN을 설치할 서버의 운영체제는 CentOS 7 이다. ( == bash)
로컬 SVN을 설치할 운영체제는 Windows 10 이다. ( == cmd )
...
[IF ) 'Job for svnserve.service failed because the control process exited with error code' ERROR 발생 시](##Job for svnserve.service failed because the control process exited with error code)
...
이후 체크아웃할 로컬 프로젝트를 선택한다. 필자는 사진과 달리 로컬 프로젝트의 최상위 디렉토리를 선택했다.
...
이후 commit하고 싶은 파일을 선택하여 commit한다.

![svn](/assets/img/post_img/svn/commit.png)

Commit 시에는 commit할 파일이 있는 경로의 디렉토리 모두 선택해야 한다.
commit을 하고 나면 하단 탭에서 commit log를 확인할 수 있다.
...
[great-artist 님 - svn : Unable to connect to a repository at URL](https://great-artist.tistory.com/67)
